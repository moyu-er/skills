...
- **[parallel-implement](./parallel-implement/SKILL.md)** — Orchestrate concurrent implementation of many already-specified tasks with dependency edges: each task dispatches the moment its predecessors land, runs in its own git worktree, and is verified, reviewed, and committed individually. Use when there are 3+ independent tasks that form a DAG.
